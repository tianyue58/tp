...
/**
 * Changes the remark of an existing application in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the application identified "
            + "by the index number used in the last application listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the application in the filtered application list to edit the remark
     * @param remark of the application to be updated to
     */
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of application data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the application’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a application.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each application.
...
``` java
public PersonCard(Person application, int displayedIndex) {
    //...
    remark.setText(application.getRemark().value);
}
```
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person applicationToEdit = lastShownList.get(index.getZeroBased());
        Person editedApplication = new Person(
                applicationToEdit.getName(), applicationToEdit.getPhone(), applicationToEdit.getEmail(),
                applicationToEdit.getAddress(), remark, applicationToEdit.getTags());

        model.setPerson(applicationToEdit, editedApplication);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedApplication));
    }

    /**
     * Generates a command execution success message based on whether
     * the remark is added to or removed from
     * {@code applicationToEdit}.
     */
    private String generateSuccessMessage(Person applicationToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, applicationToEdit);
    }
```
